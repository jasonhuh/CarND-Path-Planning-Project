        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (d < (2+4*lane + 2) && d > (2+4*lane - 2)) { // is a car within my car's range?
              ...
              if ((check_car_s > car_s) && ((check_car_s - car_s) < 30)) { // gap is less than 30 meters
                // Do some logic, lower reference velocity so we don't crash into the car in front of us,
          ...
          // Transform to local car's coordinate
          ...
          // Fill up the rest of our path planner after filling it with previous points, here we always output 
          // 50 points
          for (int i = 1; i <= 50 - previous_path_x.size(); i++) {
            double N = (target_dist/(.02*ref_vel/2.24)); //meters per sec
